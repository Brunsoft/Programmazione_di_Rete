...
cp conf/jrmp.policy policy/jrmp.policy
...
sed -i 's,_SERVERIP,'$SERVERIP',g' $PWD/policy/jrmp.policy
# Imposto il Codebase
sed -i 's,_CODEBASE,'$CODEBASE-',g' $PWD/policy/jrmp.policy

java \
-Djava.security.policy=./policy/jrmp.policy \
...
client.bootstrap.ClientJRMPBootstrap $1
